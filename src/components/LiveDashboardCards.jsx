...
function DashCard({ title, route, children }) {
  ...
  return (
    <div className="rounded-xl p-5 border flex flex-col gap-4 cursor-pointer group transition-all duration-200" style={{ background: '#161a22', borderColor: '#1e2330' }} onClick={function() { nav(route) }} onMouseEnter={function(e) { e.currentTarget.style.borderColor = 'rgba(59,130,246,0.3)' }} onMouseLeave={function(e) { e.currentTarget.style.borderColor = '#1e2330' }}>
      <h3 className="text-sm font-semibold" style={{ color: '#eceef5' }}>{title}</h3>
      <div className="space-y-2 flex-1">{children}</div>
      <button className="flex items-center gap-1.5 text-xs font-medium mt-auto" style={{ color: '#3b82f6' }}>View Details <ArrowRight size={12} /></button>
    </div>
  )
}
...
export function TechnicalCard() {
  ...
  var structure = '—'; var structureColor = '#8892a4'; var momentum = '—'
  if (change !== null) {
...
export function XRPNewsCard() {
  ...
  useEffect(function() {
    ...
    fetch('https://min-api.cryptocompare.com/data/v2/news/?categories=XRP,Ripple&excludeCategories=Sponsored&lang=EN&api_key=' + key)
      ...
      .then(function(json) { if (json && json.Data && json.Data.length > 0) setArticle(json.Data[0]) })
      .catch(function() {})
  }, [])
  function timeAgo(ts) {
...
export function DominoTheoryCard() {
  ...
  useEffect(function() {
    supabase.from('domino_theory').select('domino_number, domino_name, status').order('domino_number').then(function(res) { if (res.data) setDominoes(res.data) })
  }, [])
...
export function GeopoliticalCard() {
  ...
  useEffect(function() {
    supabase.from('geopolitical_watch').select('*').eq('section', 'flashpoint').order('sort_order', { ascending: true }).limit(3).then(function(res) { if (res.data) setItems(res.data) })
  }, [])
  var top = items[0]
...
export function MediaIntelCard() {
  ...
  useEffect(function() {
    ...
    fetch('https://min-api.cryptocompare.com/data/v2/news/?categories=XRP,Ripple,ETF,Regulation&excludeCategories=Sponsored&lang=EN&api_key=' + key)
      .then(function(r) { return r.json() })
      .then(function(json) { if (json && json.Data) setArticles(json.Data.slice(0, 10)) })
      .catch(function() {})
  }, [])
  var categories = {}
